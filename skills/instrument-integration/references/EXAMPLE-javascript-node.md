...
app.patch('/todos/:id/complete', (req, res) => {
    ...
    trackEvent(userId, 'todo_completed', {
        todo_id: todo.id,
        time_to_complete_hours: (new Date(todo.completed_at) - new Date(todo.created_at)) / 3600000,
    })
...
app.get('/stats', async (req, res) => {
    ...
    if (posthog) {
        const showDetailed = await posthog.isFeatureEnabled('detailed-analytics', userId)
