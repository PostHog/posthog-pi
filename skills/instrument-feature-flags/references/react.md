...
6.  6
    ...
    The `useFeatureFlagPayload` hook does _not_ send a `$feature_flag_called` event, which is required for experiments. Always use it with `useFeatureFlagEnabled` or `useFeatureFlagVariantKey`:
